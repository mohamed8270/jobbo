...
const JobsAnalyticsCard = ({href, icn, txt1, txt2, txt3, percentage} : Props) => {
  return (
    <Link href={href}>
        <div className='h-[120px] w-full md:w-[260px] my-[10px] md:my-[0px] bg-jwhite border-2 border-jsgrey rounded-lg font-poppins'>
            <div className='flex flex-col justify-center items-start m-[10px]'>
                <h1 className='text-jblack font-semibold text-xs'>{txt1}</h1>
                <div className='flex gap-[10px]'>
                    <Image src={icn} alt='logo' height={22} width={22} className='fill-jblack text-jblack'/>
                    <h1 className='text-lg font-normal text-jblack my-[5px] line-clamp-1'>{txt2}</h1>
                </div>
                <div className='flex justify-between items-center my-[7px] gap-[20px]'>
                    <div className='flex items-center gap-[5px]'>
                        <h1 className='text-sm font-normal text-jgreen'>{percentage}</h1>
                        <p className='text-xs font-medium text-jblack text-opacity-30'>vs last week</p>
                    </div>
                    <div>
